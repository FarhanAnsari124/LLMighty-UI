  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (
        ...
        dispatch(
          setUserData({ ...userData, aiCredits: data.remainingCredits }),
        );
      }
      setStatus("done");
      ...
      if (!err.response && !err.request) {
        ...
        message =
          "No response from server — likely a CORS error or the server is not running.";
      } else {
...
const GenerateHeader = ({ credits, isAdmin, creditCost }) => (
  ...
  >
    ...
    >
      Powered by <span className="text-[#7C6AF7]">LLMighty AI</span>
    </h1>
...
const CreditsIndicator = ({ credits, isAdmin, creditCost }) => {
  if (isAdmin) {
    return (
      <div className="inline-flex items-center gap-1.5 bg-[#28CA42]/[0.07] border border-[#28CA42]/20 rounded-full px-3.5 py-1.5">
        ...
        <span className="text-[11px] text-[#28CA42]/80 font-semibold">
          Admin · Unlimited
        </span>
      </div>
    );
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 border ${
        isDepleted
    ...
    >
      <TbCoins
        size={12}
        color={isDepleted ? "#FF5F57" : isLow ? "#FFBD2E" : "#48484A"}
      />
      <span
        className={`text-[11px] font-semibold ${isDepleted ? "text-[#FF5F57]/80" : isLow ? "text-[#FFBD2E]/80" : "text-[#48484A]"}`}
      >
        {credits} credits
      </span>
      ...
      <div className="flex items-center gap-2">
        ...
        {!hasCredits && !isAdmin && (
          <div className="flex items-center gap-1.5 text-[11px] text-[#FF5F57]/70 mr-1">
            <TbInfoCircle size={13} /> Not enough credits
          </div>
        )}
        <motion.button
          onClick={onGenerate}
          ...
          whileTap={
            prompt.trim() && !isGenerating && hasCredits ? { scale: 0.97 } : {}
          }
...
const GeneratingState = () => (
  <div className="mt-6 bg-[#080808] border border-white/[0.06] rounded-[22px] p-8 text-center">
    ...
    <p className="text-[13px] text-[#48484A] font-medium mb-5">
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-[10px] text-[#3A3A3C] mt-0.5 truncate max-w-[320px]">
          Props:{" "}
          {Array.isArray(result.props) ? result.props.join(", ") : result.props}
        </p>
      </div>
    </div>
    ...
    <div className="relative bg-[#020617] min-h-[340px] flex items-center justify-center p-8 overflow-auto">
      ...
      >
        <LiveProvider code={liveCode} scope={LIVE_SCOPE} noInline>
          <LiveError className="text-[11.5px] font-mono text-[#FF5F57] bg-[#FF5F57]/[0.07] border border-[#FF5F57]/20 rounded-xl p-4 mb-3 w-full" />
          <LivePreview />
        </LiveProvider>
      </motion.div>
    </div>
    ...
    <div className="relative">
      <div className="flex items-center justify-between px-5 py-2.5 border-b border-white/[0.04] bg-[#070707]">
        ...
        <div className="flex items-center gap-1">
          <div className="w-1.5 h-1.5 rounded-full bg-[#28CA42]" />
          <span className="text-[9.5px] text-[#28CA42]/60">
            {lines.length} lines
          </span>
        </div>
      </div>
      ...
      {genTime && (
        <>
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-[#28CA42]" />
            <span className="text-[10.5px] text-[#28CA42]/70 font-medium">
    ...
    <div className="flex items-center gap-2">
      ...
      {isAdmin && (
        <motion.button
          ...
          whileHover={!publishing ? { scale: 1.03, y: -1 } : {}}
          whileTap={!publishing ? { scale: 0.97 } : {}}
          className="flex items-center gap-2 text-[12px] font-semibold px-4 py-2 rounded-xl border transition-all disabled:opacity-50
            bg-[#28CA42]/10 border-[#28CA42]/20 text-[#28CA42] hover:bg-[#28CA42]/20 cursor-pointer disabled:cursor-not-allowed"
        >
          {publishing ? (
            <motion.div
              animate={{ rotate: 360 }}
